...
class CurriculumController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $curricula = Curriculum::with(['versions' => fn ($q) => $q->withCount('topics')])
            ->where(fn ($q) => $q->whereNull('user_id')->orWhere('user_id', auth()->user()->id))
    ...
    {
        $sources = CurriculumVersion::with(['curriculum:id,title,school_type,grades,variant'])->withCount('topics')->where('is_editable', false)->whereHas('curriculum', fn ($q) => $q->whereNull('user_id')->orWhere('user_id', auth()->user()->id))->orderBy('id')->get();
    ...
    {
        $available = Curriculum::query()->where(fn ($query) => $query->whereNull('user_id')->orWhere('user_id', auth()->user()->id))->orderBy('title')->get(['id', 'title', 'school_type', 'grades']);
        $selectedIds = collect([$request->integer('left'), $request->integer('right')])->filter()->unique()->values();
        $selected = Curriculum::with(['versions' => fn ($query) => $query->latest('id'), 'versions.topics' => fn ($query) => $query->withCount('educationPlanReferences')->orderByRaw('year is null desc')->orderBy('year')->orderBy('position')])->whereIn('id', $selectedIds)->where(fn ($query) => $query->whereNull('user_id')->orWhere('user_id', auth()->user()->id))->get()->keyBy('id');
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $curriculum = DB::transaction(function () use ($data): Curriculum {
            ...
            $sourceVersions = CurriculumVersion::with(['curriculum', 'bindings', 'topics.educationPlanReferences', 'topics.profiles', 'topics.perspectives'])->whereIn('id', $sourceVersionIds)->where('is_editable', false)->whereHas('curriculum', fn ($q) => $q->whereNull('user_id')->orWhere('user_id', auth()->user()->id))->get();
            ...
            $curriculum = Curriculum::create(['user_id' => auth()->user()->id, 'title' => $data['title'], 'school_type' => $data['school_type'] ?? null, 'grades' => $grades, 'denominations' => $denominations, 'cooperation_model' => 'confessional_cooperative', 'derived_from_id' => $sourceVersions->first()?->curriculum_id]);
    ...
    {
        $curriculumTypes = Curriculum::where(fn ($query) => $query->whereNull('user_id')->orWhere('user_id', auth()->user()->id))
            ->pluck('school_type');
        $planTypes = EducationPlan::where(fn ($query) => $query->whereNull('user_id')->orWhere('user_id', auth()->user()->id))
    ...
    {
        abort_unless($curriculum->user_id === null || $curriculum->user_id === auth()->user()->id, 404);
    }
