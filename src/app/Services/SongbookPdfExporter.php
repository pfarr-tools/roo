...
class SongbookPdfExporter
{
            ...
            foreach ($lines as $lineNumber => $line) {
                ...
                foreach ($characters as $offset => $character) {
                    ...
                    if (preg_match('/\s/u', $character)) {
                        $markup .= $chord
                            ? '<span class="chord-character"><span class="chord">'.e($chord->chord).'</span>&nbsp;</span>'
                            : ' ';

                        continue;
                    }
                    if ($offset === 0 || preg_match('/\s/u', $characters[$offset - 1] ?? '')) {
                        $markup .= '<span class="chord-word">';
                    }
                    $markup .= '<span class="chord-character">'.($chord ? '<span class="chord">'.e($chord->chord).'</span>' : '').e($character).'</span>';
                    if ($offset === count($characters) - 1 || preg_match('/\s/u', $characters[$offset + 1] ?? '')) {
                        $markup .= '</span>';
                    }
                }
    ...
    private function htmlPage(string $directory, string $title, string $content, string $format, string $name): string
    {
        ...
        $chordCss = $isChordSheet ? '.chord-instrument{font-size:10pt;margin:-1.25rem 0 1.5rem}.chord-part{white-space:normal}.chord-part-label{font-weight:bold;line-height:1;margin:0 0 .25rem}.chord-line{position:relative;min-height:calc(2em + 15pt);padding-top:15pt;line-height:2;white-space:normal;overflow-wrap:anywhere;word-break:normal}.chord-word{display:inline-block;max-width:100%;white-space:normal;vertical-align:top}.chord-character{position:relative;display:inline-block;white-space:pre}.chord{position:absolute;left:0;top:-8pt;font-family:"Atkinson Hyperlegible Next";font-size:10pt;font-weight:normal;line-height:1;white-space:nowrap}.chord-empty{position:relative;display:inline-block;top:auto;margin-bottom:.25rem}' : '';
