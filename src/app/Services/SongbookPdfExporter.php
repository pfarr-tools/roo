...
class SongbookPdfExporter
{
    ...
    {
        $parts = $version->parts->map(fn ($part) => '<section class="part '.($part->is_refrain ? 'refrain' : '').'"><h2>'.e($part->title ?: ($part->is_refrain ? 'Kehrvers' : '')).'</h2><div>'.nl2br(e($part->content)).'</div></section>')->implode('');
        ...
        return $this->htmlPage($directory, $version->song->title, '<div class="song-number">'.$number.'</div><h1>'.e($version->song->title).'</h1>'.$parts.$images, $format, 'song-'.$number);
    }
    ...
    private function htmlPage(string $directory, string $title, string $content, string $format, string $name): string
    {
        ...
        $html = '<!doctype html><html lang="de"><head><meta charset="utf-8"><style>@page{size:'.$size.';margin:'.config('songs.page_margin_mm', 12).'mm}*{box-sizing:border-box}body{font-family:'.config('songs.font_family', 'Arial, sans-serif').';font-size:'.config('songs.font_size', 12).'pt;position:relative;margin:0}h1{font-size:22pt;margin:0 0 12mm}.part{margin:0 0 7mm;white-space:normal}.part h2{font-size:13pt;margin:0 0 2mm}.refrain{font-weight:bold;border-left:4px solid '.config('songs.refrain_border_color', '#555555').';padding-left:4mm}.song-number{float:right;font-size:11pt}.placed-image{position:absolute;object-fit:contain;transform-origin:center}.title-image{width:100%;height:100%;object-fit:contain}</style></head><body>'.$content.'</body></html>';
        ...
        try {
            ...
            $this->minimalPdf($pdfPath, strip_tags($content));
        }
        return $pdfPath;
    }
