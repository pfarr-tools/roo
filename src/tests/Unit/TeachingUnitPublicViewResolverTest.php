...
it('shows immediate phase material but hides lesson-timed material before its first concrete slot', function () {
    ...
    $school = School::create(['user_id' => $user->id, 'name' => 'Public View Schule']);
    ...
    $unit = $group->teachingUnits()->create(['user_id' => $user->id, 'created_by_user_id' => $user->id, 'title' => 'Wasser des Lebens', 'position' => 1, 'introduction_text' => 'Wir arbeiten gemeinsam.']);
    ...
    $immediate = ResourceReference::create(['user_id' => $user->id, 'teaching_unit_id' => $unit->id, 'lesson_id' => $lesson->id, 'original_name' => 'Sofort.pdf', 'storage_path' => 'units/sofort.pdf', 'security_status' => 'approved']);
    $timed = ResourceReference::create(['user_id' => $user->id, 'teaching_unit_id' => $unit->id, 'lesson_id' => $lesson->id, 'original_name' => 'Später.pdf', 'storage_path' => 'units/spaeter.pdf', 'security_status' => 'approved']);
...
it('uses the earliest non-cancelled concrete slot and excludes direct unit materials', function () {
    ...
    $school = School::create(['user_id' => $user->id, 'name' => 'Slot Schule']);
    $year = SchoolYear::create(['user_id' => $user->id, 'school_id' => $school->id, 'name' => '2026/27', 'starts_on' => '2026-09-01', 'ends_on' => '2027-07-31']);
    $group = TeachingGroup::create(['user_id' => $user->id, 'school_id' => $school->id, 'school_year_id' => $year->id, 'name' => '4b']);
    $unit = $group->teachingUnits()->create(['user_id' => $user->id, 'created_by_user_id' => $user->id, 'title' => 'Slotprüfung', 'position' => 1]);
    $direct = ResourceReference::create(['user_id' => $user->id, 'teaching_unit_id' => $unit->id, 'original_name' => 'Direkt.pdf', 'storage_path' => 'units/direkt.pdf', 'security_status' => 'approved', 'publication_status' => 'shared_immediately']);
    $lesson = $unit->lessons()->create(['title' => 'Doppelstunde', 'position' => 1, 'duration' => 2]);
    $phase = $lesson->phases()->create(['title' => 'Einstieg', 'position' => 1]);
    $timed = ResourceReference::create(['user_id' => $user->id, 'teaching_unit_id' => $unit->id, 'lesson_id' => $lesson->id, 'original_name' => 'Phasenmaterial.pdf', 'storage_path' => 'units/phase.pdf', 'security_status' => 'approved']);
    $phase->resources()->attach($timed, ['publication_status' => 'shared_with_lesson']);
