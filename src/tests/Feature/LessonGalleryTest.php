...
it('uploads multiple images to the lesson gallery and serves them on the signed public page', function () {
    Storage::fake('local');
    $organization = Organization::create(['name' => 'Galerie Organisation']);
    $user = User::factory()->create(['organization_id' => $organization->id]);
    $school = School::create(['organization_id' => $organization->id, 'name' => 'Galerie Schule']);
    ...
    $unit = $group->teachingUnits()->create(['organization_id' => $organization->id, 'created_by_user_id' => $user->id, 'title' => 'Galerieeinheit', 'position' => 1]);
...
it('does not allow a gallery image from another lesson to be deleted', function () {
    $organization = Organization::create(['name' => 'Galerie Scope Organisation']);
    $user = User::factory()->create(['organization_id' => $organization->id]);
    $school = School::create(['organization_id' => $organization->id, 'name' => 'Galerie Scope Schule']);
    $year = SchoolYear::create(['organization_id' => $organization->id, 'school_id' => $school->id, 'name' => '2026/27', 'starts_on' => '2026-09-01', 'ends_on' => '2027-07-31']);
    $group = TeachingGroup::create(['organization_id' => $organization->id, 'school_id' => $school->id, 'school_year_id' => $year->id, 'name' => '4b']);
    $unit = $group->teachingUnits()->create(['organization_id' => $organization->id, 'title' => 'Scope', 'position' => 1]);
    ...
    $galleryImage = $otherLesson->galleryImages()->create(['resource_reference_id' => ResourceReference::create(['organization_id' => $organization->id, 'original_name' => 'bild.jpg', 'storage_path' => 'bild.jpg', 'mime_type' => 'image/jpeg'])->id, 'position' => 0]);
