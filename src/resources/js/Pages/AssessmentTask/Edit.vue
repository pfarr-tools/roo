...
const taskTypes = Object.entries(de.assessmentTaskTypeLabels).map(([value, label]) => ({ value, label }))
...
const newOption = () => ({ id: `option-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, text: '', correct: false })
const emptyContent = () => ({ prompt: '', lines: 5, lineated: false, reading_text: '', options: [newOption()], points_per_correct_answer: 1, checkbox_scoring_mode: 'correct_states', columns: [''], rows: [{ label: '', answer: '' }], images: [{ url: '', label: '', answer: '' }], questions: [{ label: '', lines: 3 }], words: '' })
...
function addOption() { form.content.options.push(newOption()) }
        ...
        <div class="container-full px-3 py-4">
            ...
            <form id="assessment-task-form" novalidate @submit.prevent="save">
                ...
                <section v-show="editorTab === 'content'" class="row g-4" role="tabpanel">
                    ...
                    <div v-if="usesOptions(form.task_type)" class="mt-4"><h3 class="h6">{{ de.assessmentTaskOptions }}</h3><div v-if="form.task_type === 'checkbox'" class="mb-3"><label class="form-label" for="assessment-task-points-per-correct-answer">{{ de.assessmentTaskPointsPerCorrectAnswer }}</label><input id="assessment-task-points-per-correct-answer" v-model="form.content.points_per_correct_answer" class="form-control" type="number" min="0" step="1" required><label class="form-label mt-3" for="assessment-task-checkbox-scoring-mode">{{ de.assessmentTaskCheckboxScoringMode }}</label><select id="assessment-task-checkbox-scoring-mode" v-model="form.content.checkbox_scoring_mode" class="form-select"><option value="correct_states">{{ de.assessmentTaskCheckboxScoringCorrectStates }}</option><option value="correct_selections">{{ de.assessmentTaskCheckboxScoringCorrectSelections }}</option></select></div><div v-for="(option, index) in form.content.options" :key="option.id" class="input-group mb-2"><input v-model="option.text" class="form-control" :placeholder="de.assessmentTaskOption" required><span class="input-group-text"><input v-model="option.correct" type="checkbox" class="form-check-input me-2">{{ de.assessmentTaskCorrect }}</span><button type="button" class="btn btn-outline-danger" @click="removeAt(form.content.options, index)">×</button></div><button type="button" class="btn btn-sm btn-outline-secondary" @click="addOption">{{ de.assessmentTaskAddOption }}</button></div>
