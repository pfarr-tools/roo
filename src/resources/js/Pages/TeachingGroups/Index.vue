...
const form = useForm({ school_id: '', school_year_id: '', name: '', aktenzeichen: '', denomination: '', notes: '', grade_levels: [''] })
...
async function remove(group) { if (await requestConfirmation({ message: de.deleteTeachingGroupConfirm })) router.delete(`/unterrichtsgruppen/${group.id}`) }
</script>
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <div v-for="group in groups" :key="group.id" class="card mb-3"><div class="card-body d-flex justify-content-between align-items-center gap-3"><div><h2 class="h5 mb-1"><a :href="`/unterrichtsgruppen/${group.id}`" class="text-decoration-none">{{ group.name }}</a></h2><div class="text-muted">{{ group.school.name }} · {{ group.school_year.name }} · {{ group.grade_levels.map(level => level.grade_level).join(', ') }}</div></div><div class="d-flex align-items-center gap-2"><span class="badge text-bg-light">{{ group.students_count }} {{ de.members }}</span><button class="btn btn-sm btn-outline-danger" type="button" :title="de.deleteTeachingGroup" :aria-label="de.deleteTeachingGroup" @click="remove(group)"><i class="bi bi-trash" aria-hidden="true"></i></button></div></div></div>
        </div>
        <div v-if="open" class="roo-modal-backdrop" role="presentation" @click.self="open = false"><section class="roo-modal" role="dialog" aria-modal="true" :aria-label="de.addTeachingGroup"><div class="card border-0"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-3"><h2 class="h5 mb-0">{{ de.addTeachingGroup }}</h2><button class="btn-close" type="button" :aria-label="de.close" @click="open = false"></button></div><form @submit.prevent="create">
            ...
            <label class="form-label mt-3">{{ de.aktenzeichen }}</label><select v-model="form.aktenzeichen" class="form-select"><option value="">{{ de.noAktenzeichen }}</option><option v-for="option in de.aktenzeichenOptions" :key="option.value" :value="option.value">{{ option.value }} – {{ option.label }}</option></select><div class="form-text">{{ de.aktenzeichenHint }}</div>
            <label class="form-label mt-3">{{ de.denomination }}</label><select v-model="form.denomination" class="form-select"><option value="">{{ de.choose }}</option><option v-for="option in ['evangelical', 'catholic', 'old_catholic', 'syriac_orthodox']" :key="option" :value="option">{{ de.denominationLabels[option] }}</option></select>
