...
const props = defineProps({ group: Object, students: Array, curricula: Array, schoolPeriods: Array, ritualPhaseTemplates: Array, songVersions: { type: Array, default: () => [] }, songbookVersions: { type: Array, default: () => [] }, assessments: { type: Array, default: () => [] }, reportPeriods: { type: Array, default: () => [] } })
    ...
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <Tab id="timetable" :active-tab="activeTab">
                <div class="card card-body"><h2 class="h4">{{ de.groupTimetable }}</h2><p class="text-muted">Regelmäßige Unterrichtszeiten der Gruppe.</p><div v-if="!editForm.periods.length" class="text-muted">Noch keine Unterrichtszeiten hinterlegt.</div><div v-else class="table-responsive"><table class="table table-sm"><thead><tr><th>Wochentag</th><th>Stunde</th><th>Zeitraum</th></tr></thead><tbody><tr v-for="period in editForm.periods" :key="`${period.school_period_id}-${period.weekday}`"><td>{{ weekdays.find(day => day.value === period.weekday)?.label }}</td><td>{{ schoolPeriods.find(item => item.id === period.school_period_id)?.period_number }}</td><td>{{ schoolPeriods.find(item => item.id === period.school_period_id)?.starts_at?.slice(0, 5) }}–{{ schoolPeriods.find(item => item.id === period.school_period_id)?.ends_at?.slice(0, 5) }}</td></tr></tbody></table></div><button class="btn btn-primary" type="button" @click="activeTab = 'general'">Stundenplan bearbeiten</button></div>
            </Tab>
            <Tab id="students" :active-tab="activeTab">
                <div class="card card-body"><div class="d-flex justify-content-between align-items-center mb-3"><div><h2 class="h4">{{ de.students }}</h2><p class="text-muted mb-0">{{ group.students.length }} Schüler:innen in dieser Gruppe.</p></div><button class="btn btn-primary" type="button" @click="activeTab = 'general'; showStudentModal = true">Schüler:in hinzufügen</button></div><div class="list-group"><div v-for="student in group.students" :key="student.id" class="list-group-item d-flex justify-content-between"><span>{{ student.last_name }}, {{ student.first_name }}</span><span class="text-muted">{{ student.class_name }}</span></div><div v-if="!group.students.length" class="list-group-item text-muted">Noch keine Schüler:innen zugeordnet.</div></div></div>
            </Tab>
            <Tab id="assessments" :active-tab="activeTab">
                <div class="card card-body"><div class="d-flex justify-content-between align-items-center mb-3"><div><h2 class="h4">{{ de.groupAssessments }}</h2><p class="text-muted mb-0">Lernstandserhebungen und Aufgaben dieser Gruppe.</p></div><a class="btn btn-primary" :href="`/unterrichtsgruppen/${group.id}/lernstandserhebungen`">Arbeitsbereich öffnen</a></div><div v-if="!assessments.length" class="text-muted">Noch keine Lernstandserhebung angelegt.</div><div v-for="assessment in assessments" :key="assessment.id" class="border-top py-2"><strong>{{ assessment.title }}</strong><span class="text-muted ms-2">{{ assessment.tasks.length }} Aufgaben</span></div></div>
            </Tab>
            <Tab id="evaluations" :active-tab="activeTab">
                <div class="card card-body"><div class="d-flex justify-content-between align-items-center mb-3"><div><h2 class="h4">{{ de.groupEvaluations }}</h2><p class="text-muted mb-0">Bewertungszeiträume und bearbeitbare Entwürfe.</p></div><a class="btn btn-primary" :href="`/unterrichtsgruppen/${group.id}/bewertungen`">Arbeitsbereich öffnen</a></div><div v-if="!reportPeriods.length" class="text-muted">Noch kein Bewertungszeitraum angelegt.</div><div v-for="period in reportPeriods" :key="period.id" class="border-top py-2"><strong>{{ period.label }}</strong><span class="text-muted ms-2">{{ period.evaluations?.length ?? 0 }} Entwürfe</span></div></div>
            </Tab>
            </Tabs>
        </div>
