    ...
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <Tab id="assessments" :active-tab="activeTab">
                <div class="card card-body"><div class="d-flex justify-content-between align-items-center mb-3"><div><h2 class="h4">{{ de.groupAssessments }}</h2><p class="text-muted mb-0">Lernstandserhebungen und Aufgaben dieser Gruppe.</p></div></div><div v-if="!assessments.length" class="text-muted">Noch keine Lernstandserhebung angelegt.</div><div v-for="assessment in assessments" :key="assessment.id" class="border-top py-3 d-flex justify-content-between align-items-start"><div><strong>{{ assessment.title }}</strong><span v-if="assessment.assessed_on" class="text-muted ms-2">{{ formatDate(assessment.assessed_on) }}</span><span class="d-block small text-muted">{{ assessment.tasks.length }} Aufgaben</span></div><AssessmentActions :group-id="group.id" :assessment-id="assessment.id" :title="assessment.title" /></div></div>
            </Tab>
