...
const props = defineProps({ group: Object, students: Array, curricula: Array, schoolPeriods: Array })
const selectedStudent = ref(null)
const showStudentModal = ref(false)
const showImportModal = ref(false)
const showMemberModal = ref(false)
const editForm = useForm({ school_id: props.group.school_id, school_year_id: props.group.school_year_id, name: props.group.name, notes: props.group.notes ?? '', grade_levels: props.group.grade_levels.map(level => level.grade_level), periods: (props.group.school_periods ?? []).map(period => ({ school_period_id: period.id, weekday: period.pivot.weekday })) })
...
function save() {
    ...
    editForm.put(`/unterrichtsgruppen/${props.group.id}`)
}
...
<template>
    <AppShell>
        <template #toolbar><a href="/unterrichtsgruppen" class="btn btn-sm btn-light">{{ de.teachingGroups }}</a><button class="btn btn-sm btn-outline-primary" type="button" @click="showMemberModal = true">{{ de.assignStudent }}</button><button class="btn btn-sm btn-outline-primary" type="button" @click="showStudentModal = true">{{ de.addStudent }}</button><button class="btn btn-sm btn-outline-primary" type="button" @click="showImportModal = true">{{ de.importStudents }}</button><button class="btn btn-sm btn-primary" type="button" @click="save">{{ de.saveChanges }}</button></template>
        <div class="container-full px-3 py-4">
            <a href="/unterrichtsgruppen" class="text-decoration-none">{{ de.teachingGroups }}</a>
            <h1 class="h2 mt-2">{{ group.name }}</h1><p class="text-muted">{{ group.school.name }} · {{ group.school_year.name }}</p>
