...
<template>
    <AppShell>
        <template #toolbar><a href="/unterrichtsgruppen" class="btn btn-sm btn-light" :title="de.close" :aria-label="de.close"><i class="bi bi-x-lg" aria-hidden="true"></i></a><button class="btn btn-sm btn-primary" type="button" @click="save">{{ de.saveChanges }}</button><a class="btn btn-sm btn-primary ms-2 d-inline-flex align-items-center" :href="`/unterrichtsgruppen/${group.id}/bewertungen/neu`"><i class="bi bi-plus-lg me-1" aria-hidden="true"></i>{{ de.addEvaluationPeriod }}</a></template>
        <div class="container-full px-3 py-4">
            ...
            <Tab id="students" :active-tab="activeTab">
                <section class="card card-body"><div class="d-flex justify-content-between align-items-center gap-2 mb-3"><h2 class="h5 mb-0">{{ de.members }} ({{ group.students.length }})</h2><div class="d-flex gap-1"><button class="btn btn-sm btn-outline-primary" type="button" @click="showMemberModal = true">{{ de.assign }}</button><button class="btn btn-sm btn-outline-primary" type="button" @click="showStudentModal = true">{{ de.create }}</button><button class="btn btn-sm btn-outline-primary" type="button" @click="showImportModal = true">{{ de.import }}</button></div></div><div v-if="!group.students.length" class="text-muted mb-3">{{ de.noMembers }}</div><div v-for="student in group.students" :key="student.id" class="border-bottom py-2"><div class="d-flex justify-content-between align-items-center"><a :href="'/schueler:innen/' + student.id" class="text-decoration-none">{{ student.last_name }}, {{ student.first_name }}</a> <small class="text-muted">({{ student.class_name }})</small><div class="d-flex gap-1"><button class="btn btn-sm btn-outline-secondary" type="button" :title="de.editStudent" @click="editStudent(student)"><i class="bi bi-pencil" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-danger" type="button" :title="de.removeAssignment" @click="remove(student)"><i class="bi bi-x-lg" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-danger" type="button" :title="de.deleteStudent" @click="deleteStudent(student)"><i class="bi bi-trash" aria-hidden="true"></i></button></div></div><small v-if="student.pivot?.starts_on || student.pivot?.ends_on" class="text-muted">{{ student.pivot?.starts_on ?? '–' }} – {{ student.pivot?.ends_on ?? '–' }}</small></div></section>
            </Tab>
