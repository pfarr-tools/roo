...
<template>
    <AppShell>
        <template #toolbar><a href="/unterrichtsgruppen" class="btn btn-sm btn-light" :title="de.close" :aria-label="de.close"><i class="bi bi-x-lg" aria-hidden="true"></i></a><button class="btn btn-sm btn-primary" type="button" @click="save">{{ de.saveChanges }}</button><a v-if="group.grading_model === 'competency_texts_and_grades'" class="btn btn-sm btn-primary ms-2 d-inline-flex align-items-center" :href="`/unterrichtsgruppen/${group.id}/bewertungen/neu`"><i class="bi bi-plus-lg me-1" aria-hidden="true"></i>{{ de.addEvaluationPeriod }}</a></template>
        <div class="container-full px-3 py-4">
            ...
            <Tab id="evaluations" :active-tab="activeTab">
                ...
                <section v-if="group.grading_model === 'competency_texts_and_grades'" class="card card-body mb-4">
                    <div class="d-flex justify-content-between align-items-start gap-3 mb-3"><div><h2 class="h5 mb-1">{{ de.evaluationPeriods }}</h2><p class="text-muted mb-0">{{ de.proposedEvaluationText }}</p></div></div>
                    <div v-if="!reportPeriods.length" class="text-muted">{{ de.noEvaluationTemplate }}</div>
                    <div v-for="period in reportPeriods" :key="period.id" class="border-top py-3 d-flex justify-content-between align-items-start gap-3">
                        <div><strong>{{ period.label }}</strong><div v-if="!period.evaluation_templates?.length" class="small text-muted mt-1">{{ de.noEvaluationTemplate }}</div><div v-else class="small text-muted mt-1">{{ period.evaluation_templates.map(template => template.level ? `${template.level}-Niveau` : de.proposedEvaluationText).join(', ') }}</div></div>
                        <a class="btn btn-sm btn-outline-primary" :href="`/unterrichtsgruppen/${group.id}/bewertungen/zeiträume/${period.id}/vorlage`">{{ de.editEvaluationTemplates }}</a>
                    </div>
                </section>
            </Tab>
            </Tabs>
