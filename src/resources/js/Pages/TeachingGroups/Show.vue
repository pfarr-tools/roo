...
const ritualForm = useForm({ phase_template_ids: (props.group.rituals ?? []).map(ritual => ritual.phase_template_id) })
...
function saveRituals() {
    ritualForm.put(`/unterrichtsgruppen/${props.group.id}/rituale`)
}
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <section class="card card-body mb-4"><div class="d-flex justify-content-between align-items-start gap-3"><div><h2 class="h5 mb-1">{{ de.groupCurricula }}</h2><p class="text-muted mb-0">{{ de.groupCurriculaIntro }}</p></div><button class="btn btn-sm btn-outline-primary" type="button" @click="showCurriculumModal = true">{{ de.curriculumAssignments }}</button></div><div v-if="!group.curricula?.length" class="text-muted mt-3">{{ de.noGroupCurricula }}</div><div v-else class="d-flex flex-wrap gap-2 mt-3"><span v-for="curriculum in group.curricula" :key="curriculum.id" class="badge text-bg-light">{{ curriculum.title }} · {{ curriculum.pivot.role === 'primary' ? de.primary : de.additional }}</span></div></section>
            <section class="card card-body mb-4"><div class="d-flex justify-content-between align-items-start gap-3"><div><h2 class="h5 mb-1">{{ de.groupRituals }}</h2><p class="text-muted mb-0">{{ de.groupRitualsIntro }}</p></div><button class="btn btn-sm btn-primary" type="button" :disabled="ritualForm.processing" @click="saveRituals">{{ de.saveChanges }}</button></div><div v-if="!ritualPhaseTemplates?.length" class="text-muted mt-3">{{ de.noPhaseTemplates }}</div><div v-else class="mt-3"><label class="form-label">{{ de.phaseTemplates }}</label><select v-model="ritualForm.phase_template_ids" class="form-select" multiple size="5"><option v-for="template in ritualPhaseTemplates" :key="template.id" :value="template.id">{{ template.title }}<template v-if="template.duration_minutes"> · {{ template.duration_minutes }} Min.</template></option></select><div class="form-text">{{ de.groupRitualsSelectionHint }}</div></div></section>
