...
const songbookForm = useForm({ song_version_ids: (props.group.songbook?.entries ?? []).map(entry => entry.song_version_id) })
...
function uploadTitlePage() {
    ...
    titlePageForm.post(`/unterrichtsgruppen/${props.group.id}/liederbuch/titelseite`, { forceFormData: true, preserveScroll: true, onSuccess: () => titlePageForm.reset('title_page') })
}
function saveSongbook() { songbookForm.put(`/unterrichtsgruppen/${props.group.id}/liederbuch/lieder`, { preserveScroll: true }) }
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <section class="card card-body mb-4"><div class="d-flex justify-content-between align-items-start gap-3"><div><h2 class="h5 mb-1">Gruppenliederbuch</h2><p class="text-muted mb-0">{{ group.songbook?.entries?.length ?? 0 }} Lieder · automatische Nummerierung</p></div><a v-if="group.songbook?.title_page_path" class="btn btn-sm btn-outline-secondary" :href="`/unterrichtsgruppen/${group.id}/liederbuch/titelseite`" target="_blank">Titelseite ansehen</a></div><form class="row g-2 align-items-end mt-2" @submit.prevent="uploadTitlePage"><div class="col-md-8"><label class="form-label">Titelseite hochladen (PDF oder Bild)</label><input class="form-control" type="file" accept="application/pdf,image/jpeg,image/png" @change="titlePageForm.title_page = $event.target.files?.[0] ?? null"></div><div class="col-auto"><button class="btn btn-outline-primary" type="submit" :disabled="titlePageForm.processing">Titelseite speichern</button></div></form><div v-if="group.songbook?.entries?.length" class="d-flex flex-wrap gap-2 mt-3"><span v-for="entry in group.songbook.entries" :key="entry.id" class="badge text-bg-light">{{ entry.song_number }} · {{ entry.song_version?.song?.title }}</span></div></section>
        </div>
        ...
        <section class="container-full px-3 pb-4"><div class="card card-body"><div class="d-flex justify-content-between align-items-center"><div><h2 class="h5 mb-1">Lieder zum Schuljahresbeginn</h2><p class="small text-muted mb-0">Diese Lieder starten im Gruppenliederbuch.</p></div><div class="d-flex gap-2"><a class="btn btn-sm btn-outline-secondary" :href="`/unterrichtsgruppen/${group.id}/liederbuch/export?format=a5`">A5 drucken</a><a class="btn btn-sm btn-outline-secondary" :href="`/unterrichtsgruppen/${group.id}/liederbuch/export?format=new`">Neue Seiten</a></div></div><select v-model="songbookForm.song_version_ids" class="form-select mt-3" multiple size="6"><option v-for="version in songVersions" :key="version.id" :value="version.id">{{ version.song?.title }} · {{ version.name }}</option></select><button class="btn btn-primary mt-2" type="button" :disabled="songbookForm.processing" @click="saveSongbook">Ausgangsbestand speichern</button></div></section>
    </AppShell>
</template>
