...
function saveEditor({ generate = false } = {}) {
    editor.put(`/lieder/fassungen/${editorVersion.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            refreshEditorVersion()
            if (!generate) editorVersion.value = null
        },
    })
}
function uploadImages() {
    imageForm.post(`/lieder/fassungen/${editorVersion.value.id}/bilder`, { forceFormData: true, preserveScroll: true, onSuccess: () => { refreshEditorVersion(); imageForm.reset() } })
}
function generateSheet() {
    editor.put(`/lieder/fassungen/${editorVersion.value.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            router.post(`/lieder/fassungen/${editorVersion.value.id}/liedblatt/erzeugen`, {}, {
                preserveScroll: true,
                onSuccess: () => { generatedSheetUrl.value = `/lieder/fassungen/${editorVersion.value.id}/liedblatt/erzeugt`; refreshEditorVersion() },
                onError: errors => window.alert(Object.values(errors)[0] ?? 'Das A5-Liedblatt konnte nicht erzeugt werden.'),
            })
        },
    })
}
...
function continueInteraction(event) {
    ...
    if (state.type === 'resize') {
        ...
        state.image.x = Math.max(0, Math.min(canvas.width - width, west ? state.x + state.width - width : state.x))
        state.image.y = Math.max(0, Math.min(canvas.height - height, north ? state.y + state.height - height : state.y))
        return
    }
...
<template>
    <AppShell>
        ...
        <div v-if="editorVersion" class="roo-modal-backdrop" @click.self="editorVersion = null"><section class="roo-modal roo-modal-song-editor card" role="dialog" aria-modal="true" aria-label="Lied bearbeiten"><div class="card-body d-flex flex-column"><div class="d-flex justify-content-between align-items-center"><h2 class="h5 mb-0">Lied bearbeiten</h2><button class="btn-close" type="button" @click="editorVersion = null"></button></div><ul class="nav nav-tabs mt-3 mb-3"><li class="nav-item"><button class="nav-link" :class="{ active: editorTab === 'metadata' }" type="button" @click="editorTab = 'metadata'">Metadaten</button></li><li class="nav-item"><button class="nav-link" :class="{ active: editorTab === 'parts' }" type="button" @click="editorTab = 'parts'">Liedteile</button></li><li class="nav-item"><button class="nav-link" :class="{ active: editorTab === 'layout' }" type="button" @click="editorTab = 'layout'">A5-Gestaltung</button></li></ul><div v-if="editorTab === 'metadata'" class="row g-3 editor-tab-content"><div class="col-md-8"><label class="form-label">Titel</label><input v-model="editor.song.title" class="form-control" required></div><div class="col-md-4"><label class="form-label">Sprache</label><input v-model="editor.language" class="form-control"></div><div class="col-md-6"><label class="form-label">Text:in</label><input v-model="editor.song.author" class="form-control"></div><div class="col-md-6"><label class="form-label">Komponist:in</label><input v-model="editor.song.composer" class="form-control"></div><div class="col-md-6"><label class="form-label">Fassungsname</label><input v-model="editor.name" class="form-control" required></div><div class="col-md-6"><label class="form-label">Altersgruppe</label><input v-model="editor.song.age_group" class="form-control"></div><div class="col-md-6"><label class="form-label">Themen</label><input v-model="editor.song.topics" class="form-control"></div><div class="col-md-6"><label class="form-label">Rechtehinweis</label><input v-model="editor.song.copyright_notice" class="form-control"></div><div class="col-12"><label class="form-label">Notizen</label><textarea v-model="editor.song.notes" class="form-control" rows="5"></textarea></div></div><div v-else-if="editorTab === 'parts'" class="row g-4 editor-tab-content overflow-hidden"><div class="col-lg-6 song-parts-scroll"><div class="d-flex justify-content-between align-items-center"><h3 class="h6 mb-0">Liedteile</h3><button class="btn btn-sm btn-outline-primary" type="button" @click="addPart">Teil hinzufügen</button></div><div v-for="(part, index) in editor.parts" :key="index" class="border rounded p-2 mt-2"><label class="form-check small"><input v-model="part.is_refrain" class="form-check-input" type="checkbox"> Kehrvers</label><textarea v-model="part.content" class="form-control mt-2" rows="6" placeholder="Text"></textarea></div></div><div class="col-lg-6 song-preview-pane"><h3 class="h6">A5-Vorschau</h3><div class="song-canvas border bg-white position-relative"><div class="p-3"><h4 class="song-preview-title" :style="previewStyles.title">{{ editor.song.title }}</h4><section v-for="(part, index) in editor.parts" :key="index" class="song-preview-part" :style="part.is_refrain ? previewStyles.refrain : previewStyles.text">{{ part.content }}</section></div></div></div></div><div v-else class="row g-4 editor-tab-content overflow-hidden"><div class="col-lg-5 song-parts-scroll"><label class="form-label">Bilder hinzufügen</label><input class="form-control" type="file" accept="image/*" multiple @change="imageForm.images = [...$event.target.files]"><button class="btn btn-sm btn-outline-secondary mt-2" type="button" :disabled="imageForm.processing || !imageForm.images.length" @click="uploadImages">Bilder hochladen</button><div v-for="image in editorVersion.images" :key="image.id" class="small mt-2"><button class="btn btn-sm btn-outline-secondary" type="button" @click="addImage(image)">{{ image.original_name }} auf die Vorschau legen</button></div></div><div class="col-lg-7 song-preview-pane"><h3 class="h6">A5-Vorschau</h3><div class="song-canvas border bg-white position-relative" @click="activeImageId = null"><div class="p-3"><h4 class="song-preview-title" :style="previewStyles.title">{{ editor.song.title }}</h4><section v-for="(part, index) in editor.parts" :key="index" class="song-preview-part" :style="part.is_refrain ? previewStyles.refrain : previewStyles.text">{{ part.content }}</section></div><div v-for="image in editor.layout_data.images" :key="image.id" class="song-image-selection" :class="{ selected: activeImageId === image.id }" :style="{ left: `${image.x}px`, top: `${image.y}px`, width: `${image.width}px`, height: `${image.height}px` }" @pointerdown.stop="beginInteraction('move', $event, image)"><img :src="`/lieder/fassungen/${editorVersion.id}/bilder/${image.id}`" :style="imageStyle(image)" alt=""><button v-for="corner in ['nw', 'ne', 'sw', 'se']" :key="corner" class="image-handle resize-handle" :class="corner" type="button" @pointerdown.stop="beginInteraction('resize', $event, image, corner)"></button><button class="image-handle rotate-handle" type="button" @pointerdown.stop="beginInteraction('rotate', $event, image)"></button></div></div><p v-if="generatedSheetUrl" class="mt-2 mb-0"><a class="btn btn-sm btn-success" :href="generatedSheetUrl" target="_blank"><i class="bi bi-download me-1"></i>A5-PDF herunterladen</a></p></div></div><div class="d-flex justify-content-end gap-2 mt-4"><button class="btn btn-outline-secondary" type="button" @click="editorVersion = null">Abbrechen</button><button class="btn btn-outline-primary" type="button" :disabled="editor.processing" @click="generateSheet">A5-PDF erzeugen</button><button class="btn btn-primary" type="button" :disabled="editor.processing" @click="saveEditor">Speichern</button></div></div></section></div>
    </AppShell>
</template>
