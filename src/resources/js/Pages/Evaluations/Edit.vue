...
const standardScaleLabels = [1, 2, 3, 4, 5]
...
<template>
    <AppShell>
        <template #toolbar>
            <a :href="`/bewertungen?group=${group.id}`" class="btn btn-sm btn-light" :title="de.close" :aria-label="de.close"><i class="bi bi-x-lg" aria-hidden="true"></i></a>
            <button v-if="evaluation.status !== 'confirmed'" class="btn btn-sm btn-primary ms-2" type="submit" form="evaluation-form" :disabled="form.processing">{{ de.save }}</button>
        ...
        <div class="container-full px-3 py-4">
            ...
            <section v-if="['competency_texts_and_grades', 'grades_only'].includes(group.grading_model)" class="mb-4" aria-labelledby="evaluation-lses-heading">
                <h2 id="evaluation-lses-heading" class="h5">{{ de.evaluationLses }}</h2>
                <div v-if="lses.length" class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>{{ de.title }}</th><th>{{ de.evaluationLseDate }}</th><th>{{ de.evaluationStudentLevel }}</th></tr></thead><tbody><tr v-for="lse in lses" :key="lse.id"><td>{{ lse.title }}</td><td>{{ formatDate(lse.date) }}</td><td>{{ lse.student_levels?.join(', ') || '–' }}</td></tr></tbody></table></div>
                <p v-else class="small text-muted mb-2">{{ de.noEvaluationsInPeriod }}</p>
                <label class="form-label mt-3" for="evaluation-level">{{ de.evaluationPeriodLevel }}</label>
                <select id="evaluation-level" v-model="form.level" class="form-select" :disabled="evaluation.status === 'confirmed'"><option value="">{{ de.notSet }}</option><option v-for="level in ['G', 'M', 'E']" :key="level" :value="level">{{ level }}</option></select>
            </section>
            <form id="evaluation-form" class="card card-body" @submit.prevent="save">
                <template v-if="group.grading_model !== 'observation_scales'"><label class="form-label" for="draft-text">{{ de.evaluationDraft }}</label><textarea id="draft-text" v-model="form.draft_text" class="form-control" rows="12" :disabled="evaluation.status === 'confirmed'"></textarea></template>
                <div v-if="competencies.length" class="mt-4"><h2 class="h5">{{ de.competencies }}</h2><div v-for="competence in competencies" :key="competence.id" class="border-top py-3"><div class="mb-2">{{ competence.label || competence.text }}</div><div class="d-flex gap-2" :aria-label="de.competenceRatingScale"><button v-for="level in standardScaleLabels" :key="level" class="btn btn-sm" :class="standardAverageFor(competence)?.rounded_level === level ? 'bg-primary-subtle text-primary-emphasis border-primary' : 'btn-outline-secondary'" type="button" disabled :aria-label="de.competenceRatingLevel(level)"><i class="bi bi-star-fill" aria-hidden="true"></i></button></div></div></div>
