...
function savePlanning() {
    saveForm.transform(() => ({ title: props.lesson.title, duration: props.lesson.duration, learning_goals: props.lesson.learning_goals, materials: props.lesson.materials, homework: props.lesson.homework, assessment_note: props.lesson.assessment_note, notes: props.lesson.notes, phases: phaseDraft.value, resource_links: resourceLinks.value, material_items: resourceMaterialItems.value })).put(`/jahresplanung/${props.group.id}/lessons/${props.lesson.id}`, { preserveScroll: true })
}
        ...
        <div class="container-full px-3 py-4">
            ...
            <section v-if="activeView === 'planning'" aria-labelledby="planning-heading">
                ...
                <div class="row g-4 mb-4">
                    ...
                    <div class="col-lg-6"><article class="card h-100"><div class="card-body"><h2 class="h5">{{ de.materials }}</h2><AttachmentList :resources="unit.resources ?? []" :resource-links="resourceLinks" :material-items="resourceMaterialItems" :material-text="lesson.materials" :manage="true" :upload-url="`/jahresplanung/${group.id}/eigene-einheiten/${unit.id}/anhaenge`" :upload-lesson-id="lesson.id" :download-base-url="`/jahresplanung/${group.id}/eigene-einheiten/${unit.id}/anhaenge`" @update="updateResourceDescription" @delete="deleteResource" @update:resource-links="resourceLinks = $event" @update:material-items="resourceMaterialItems = $event" @error="addToast('error', $event)" /></div></article></div>
                </div>
                <article class="card planning-phases-workspace"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-3"><div><h2 class="h4 mb-1">{{ de.phases }}</h2><p class="text-muted mb-0">{{ de.lessonPhasesWorkspaceIntro }}</p></div><span class="badge text-bg-light">{{ phaseDraft.length }} {{ de.phases.toLowerCase() }}</span></div><LessonPhasesTab :lesson="lesson" :phases="phaseDraft" :group-id="group.id" :phase-templates="phaseTemplates" :social-forms="socialForms" :resources="unit.resources ?? []" :resource-links="resourceLinks" :material-items="resourceMaterialItems" @update:phases="phaseDraft = $event" /></div></article>
            </section>
