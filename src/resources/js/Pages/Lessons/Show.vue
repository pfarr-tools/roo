...
function setCustomScale(student, competence, label) { const row = observationRow(student); const existing = customEvidence(student, competence); const evidence = existing ?? { custom_process_competence_id: competence.id, custom_scale_level: null, custom_scale_status: null, note: '' }; evidence.custom_process_competence_id = competence.id; evidence.custom_scale_level = label === 'ne' ? null : customScaleLabels.value.indexOf(label) + 1; evidence.custom_scale_status = label === 'ne' ? 'ne' : null; if (!existing) row.evidences.push(evidence) }
    ...
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <Tab id="observation" :active-tab="activeTab">
                ...
                <div class="card"><div class="table-responsive"><table class="table table-hover align-middle mb-0 observation-table"><thead><tr><th scope="col">Schüler:in</th><th scope="col">Anwesenheit</th><th v-for="type in observationTypes" :key="type.id" scope="col" class="text-center" :title="type.label">{{ type.symbol || type.label }}</th><th v-for="competence in customProcessCompetences" :key="`custom-head-${competence.id}`" scope="col" class="text-center" :title="competence.text">{{ competence.position }}</th><th scope="col">Notiz</th></tr></thead><tbody><tr v-for="student in observationStudents" :key="student.id"><th scope="row"><span>{{ student.last_name }}, {{ student.first_name }}</span><small v-if="student.class_name" class="d-block text-muted">{{ student.class_name }}</small></th><td><select v-model="observationRow(student).attendance" class="form-select form-select-sm"><option value="present">anwesend</option><option value="late">verspätet</option><option value="absent">abwesend</option></select></td><td v-for="type in observationTypes" :key="type.id" class="text-center"><button type="button" class="btn btn-sm" :class="observationRow(student).observation_type_ids.includes(type.id) ? 'btn-primary' : 'btn-outline-secondary'" :aria-pressed="observationRow(student).observation_type_ids.includes(type.id)" :title="type.label" @click="toggleObservation(student, type.id)">{{ type.symbol || '✓' }}</button></td><td v-for="competence in customProcessCompetences" :key="`custom-cell-${competence.id}`" class="text-center"><div class="d-flex flex-wrap justify-content-center gap-1"><button v-for="label in customScaleLabels" :key="label" type="button" class="btn btn-sm px-1" :class="customEvidence(student, competence)?.custom_scale_status === label || (label !== 'ne' && customEvidence(student, competence)?.custom_scale_level === customScaleLabels.indexOf(label) + 1) ? 'btn-primary' : 'btn-outline-secondary'" :aria-label="`${competence.text}: ${label}`" :aria-pressed="customEvidence(student, competence)?.custom_scale_status === label || (label !== 'ne' && customEvidence(student, competence)?.custom_scale_level === customScaleLabels.indexOf(label) + 1)" @click="setCustomScale(student, competence, label)">{{ label }}</button></div></td><td><input v-model="observationRow(student).note" class="form-control form-control-sm" maxlength="2000"></td></tr><tr v-if="!observationStudents.length"><td colspan="99" class="text-muted">Für diese Gruppe sind keine Schüler:innen erfasst.</td></tr></tbody></table></div></div>
            </Tab>
