...
async function printLessonSongs() {
    ...
    try {
        const response = await fetch(`/unterricht/${props.slot.id}/lieder/export?format=${lessonSongPrintFormat.value}`, { headers: { Accept: 'application/pdf' } })
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <Tab id="observation" :active-tab="activeTab"><div class="card"><div class="card-body"><h2 class="h4">{{ de.lessonObservation }}</h2><p class="text-muted">{{ de.lessonObservationComingSoon }}</p></div></div></Tab>
            </Tabs>
        </div>
