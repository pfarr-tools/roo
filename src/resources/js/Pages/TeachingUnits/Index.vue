...
const templateForm = useForm({ lesson_template_id: '', title: '', duration_minutes: null, description: '', material: '' })
...
function editTemplate(template) { templateEditing.value = template; templateForm.defaults({ lesson_template_id: template.lesson_template_id, title: template.title, duration_minutes: template.duration_minutes, description: template.description ?? '', material: template.material ?? '' }); templateForm.reset(); templateForm.clearErrors() }
    ...
    <AppShell>
        <div class="container-full px-3 py-4">
            ...
            <section class="mt-5" aria-labelledby="phase-templates-heading">
                ...
                <div v-for="template in phaseTemplates" :key="template.id" class="card mb-2 planning-card"><div class="card-body d-flex justify-content-between gap-3"><div><h3 class="h6 mb-1">{{ template.title }}</h3><div class="small text-muted">{{ template.duration_minutes ? `${template.duration_minutes} ${de.minutes}` : de.noDuration }} · {{ de.version }} {{ template.version }}</div><p v-if="template.description" class="small mb-0 mt-2">{{ template.description }}</p></div><div class="d-flex gap-2 align-items-start"><button class="btn btn-sm btn-outline-primary" type="button" @click="editTemplate(template)"><i class="bi bi-pencil me-1" aria-hidden="true"></i>{{ de.editPhaseTemplate }}</button><button class="btn btn-sm btn-outline-danger" type="button" @click="removeTemplate(template)"><i class="bi bi-trash" aria-hidden="true"></i><span class="visually-hidden">{{ de.deletePhaseTemplate }}</span></button></div></div></div>
            </section>
        </div>
