...
function queryUrl(changes = {}) {
    ...
    return `/schuelerinnen?${params.toString()}`
}
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <form method="get" action="/schuelerinnen" class="card card-body mb-4" role="search"><div class="row g-2 align-items-end"><div class="col-lg-5"><label class="form-label" for="student-search">{{ de.search }}</label><input id="student-search" name="q" :value="filters.q" class="form-control" :placeholder="de.searchStudents"></div><div class="col-lg-3"><label class="form-label" for="student-school">{{ de.school }}</label><select id="student-school" name="school_id" class="form-select"><option value="">{{ de.allSchools }}</option><option v-for="school in schools" :key="school.id" :value="school.id" :selected="String(filters.school_id) === String(school.id)">{{ school.name }}</option></select></div><div class="col-lg-2"><label class="form-label" for="student-class">{{ de.studentClass }}</label><select id="student-class" name="class_name" class="form-select"><option value="">{{ de.allClasses }}</option><option v-for="className in classes" :key="className" :value="className" :selected="filters.class_name === className">{{ className }}</option></select></div><div class="col-lg-2 d-flex gap-2"><button class="btn btn-primary flex-grow-1" type="submit">{{ de.search }}</button><a href="/schuelerinnen" class="btn btn-outline-secondary" :title="de.clearFilters" :aria-label="de.clearFilters"><i class="bi bi-x-lg" aria-hidden="true"></i></a></div></div></form>
            <div v-if="!students.data.length" class="alert alert-info">{{ de.noStudents }}</div>
            <div v-else class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th><a :href="sortUrl('last_name')" class="text-decoration-none" :title="sortLabel('last_name')">{{ de.lastName }} <i v-if="filters.sort === 'last_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('first_name')" class="text-decoration-none" :title="sortLabel('first_name')">{{ de.firstName }} <i v-if="filters.sort === 'first_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('class_name')" class="text-decoration-none" :title="sortLabel('class_name')">{{ de.studentClass }} <i v-if="filters.sort === 'class_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('school')" class="text-decoration-none" :title="sortLabel('school')">{{ de.studentSchool }} <i v-if="filters.sort === 'school'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th></tr></thead><tbody><tr v-for="student in students.data" :key="student.id"><td>{{ student.last_name }}</td><td>{{ student.first_name }}</td><td>{{ student.class_name }}</td><td>{{ student.school.name }}</td></tr></tbody></table></div>
