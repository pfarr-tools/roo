...
const exportFields = ref(['last_name', 'first_name', 'first_name_plus', 'pronoun_set', 'class_name', 'denomination'])
const exportSort = ref('last_name')
const exportDirection = ref('asc')
const exportGroupByClass = ref(false)
...
function openStudent(student) { window.location.href = `/schueler:innen/${student.id}` }
</script>
...
<template>
    <AppShell>
        ...
        <div class="container-full px-3 py-4">
            ...
            <div v-else class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th><a :href="sortUrl('last_name')" class="text-decoration-none" :title="sortLabel('last_name')">{{ de.lastName }} <i v-if="filters.sort === 'last_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('first_name')" class="text-decoration-none" :title="sortLabel('first_name')">{{ de.firstName }} <i v-if="filters.sort === 'first_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('class_name')" class="text-decoration-none" :title="sortLabel('class_name')">{{ de.studentClass }} <i v-if="filters.sort === 'class_name'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th><a :href="sortUrl('school')" class="text-decoration-none" :title="sortLabel('school')">{{ de.studentSchool }} <i v-if="filters.sort === 'school'" :class="filters.direction === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down'" aria-hidden="true"></i></a></th><th>{{ de.schoolYear }}</th><th><span class="visually-hidden">{{ de.actions }}</span></th></tr></thead><tbody><tr v-for="student in students.data" :key="student.id" role="link" tabindex="0" @click="openStudent(student)" @keydown.enter="openStudent(student)"><td>{{ student.last_name }}</td><td>{{ student.first_name }}</td><td>{{ student.class_name }}</td><td>{{ student.school.name }}</td><td>{{ [...new Set(student.teaching_groups.map(group => group.school_year?.name).filter(Boolean))].join(', ') || '–' }}</td><td class="text-end"><button class="btn btn-sm btn-outline-secondary me-1" type="button" :title="de.editStudent" :aria-label="de.editStudent" @click.stop="editStudent(student)"><i class="bi bi-pencil" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-danger" type="button" :title="de.deleteStudent" :aria-label="de.deleteStudent" @click.stop="deleteStudent(student)"><i class="bi bi-trash" aria-hidden="true"></i></button></td></tr></tbody></table></div>
        ...
        <div v-if="showExportModal" class="roo-modal-backdrop" role="presentation" @click.self="showExportModal = false">
            ...
            <form method="get" action="/schueler:innen/export">
                            <div class="row g-3">
                                ...
                                <fieldset class="col-12"><legend class="form-label mb-2">{{ de.exportFields }}</legend><div class="row g-2"> <div v-for="field in [{ value: 'last_name', label: de.lastName }, { value: 'first_name', label: de.firstName }, { value: 'first_name_plus', label: de.firstNamePlus }, { value: 'pronoun_set', label: de.pronouns }, { value: 'class_name', label: de.studentClass }, { value: 'denomination', label: de.denomination }, { value: 'school', label: de.studentSchool }, { value: 'school_year', label: de.schoolYears }]" :key="field.value" class="col-sm-6 col-lg-3"><div class="form-check"><input :id="`export-field-${field.value}`" v-model="exportFields" class="form-check-input" type="checkbox" name="fields[]" :value="field.value"><label class="form-check-label" :for="`export-field-${field.value}`">{{ field.label }}</label></div></div></div></fieldset>
                                <div class="col-md-6"><label class="form-label" for="export-sort">{{ de.exportSort }}</label><select id="export-sort" v-model="exportSort" name="sort" class="form-select"><option value="last_name">{{ de.lastName }}</option><option value="first_name">{{ de.firstName }}</option><option value="class_name">{{ de.studentClass }}</option><option value="school">{{ de.studentSchool }}</option><option value="school_year">{{ de.schoolYears }}</option><option value="denomination">{{ de.denomination }}</option><option value="pronoun_set">{{ de.pronouns }}</option></select></div>
                                <div class="col-md-6"><label class="form-label" for="export-direction">{{ de.exportSortDirection }}</label><select id="export-direction" v-model="exportDirection" name="direction" class="form-select"><option value="asc">{{ de.sortAscending }}</option><option value="desc">{{ de.sortDescending }}</option></select></div>
                                <div class="col-12"><div class="form-check"><input id="export-group-by-class" v-model="exportGroupByClass" class="form-check-input" type="checkbox" name="group_by_class" value="1"><label class="form-check-label" for="export-group-by-class">{{ de.exportGroupByClass }}</label></div></div>
                            </div>
