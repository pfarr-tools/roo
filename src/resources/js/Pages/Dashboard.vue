...
function slotStatusLabel(status) {
    return ({ absent: de.absent, cancelled: de.cancelled, blocked: de.blocked }[status] ?? null)
}

function cardStatusClass(entry) {
    if (slotStatusLabel(entry.slot_status)) {
        return `dashboard-lesson-card-${entry.slot_status}`
    }

    return entry.lesson ? 'dashboard-lesson-card-planned' : 'dashboard-lesson-card-unplanned'
}

function badgeClass(entry) {
    return ({ absent: 'text-bg-danger', cancelled: 'text-bg-warning', blocked: 'text-bg-secondary' }[entry.slot_status] ?? 'text-bg-success')
}

function displayStatusLabel(entry) {
    return slotStatusLabel(entry.slot_status) ?? lessonStatusLabel(entry.lesson?.status)
}

function hourUrl(entry) {
    return entry.schedule_slot_id && entry.lesson ? `/unterricht/${entry.schedule_slot_id}` : null
}
...
function navigateToWeek() {
    if (selectedWeek.value) {
        window.location.href = `/stundenplan?week=${selectedWeek.value}`
    }
}

function weekUrl(value) {
    return value ? `/stundenplan?week=${value}` : '/stundenplan'
}
</script>
