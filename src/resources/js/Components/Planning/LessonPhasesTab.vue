...
const phaseForm = ref({ title: '', duration_minutes: null, social_form: '', teacher_interaction: '', learner_activity: '', differentiation: '', didactic_comment: '', resource_ids: [], resource_link_ids: [], resource_publication_statuses: {}, resource_link_publication_statuses: {}, material_item_ids: [], song_ids: [] })
...
function addPhase() {
    ...
    phases.value.push({
        ...
        resource_ids: [], resource_link_ids: [], resource_publication_statuses: {}, resource_link_publication_statuses: {}, material_item_ids: [], song_ids: [],
    })
...
function editPhase(phase) {
    ...
    phaseForm.value = { title: phase.title ?? '', duration_minutes: phase.duration_minutes ?? null, social_form: phase.social_form?.name ?? phase.social_form ?? '', teacher_interaction: phase.teacher_interaction ?? '', learner_activity: phase.learner_activity ?? '', differentiation: phase.differentiation ?? '', didactic_comment: phase.didactic_comment ?? '', resource_ids: [...(phase.resource_ids ?? [])], resource_link_ids: [...(phase.resource_link_ids ?? [])], resource_publication_statuses: Object.fromEntries((phase.resources ?? []).map(resource => [resource.id, resource.pivot?.publication_status ?? 'not_shared'])), resource_link_publication_statuses: Object.fromEntries((phase.resource_links ?? []).map(link => [link.id, link.pivot?.publication_status ?? 'not_shared'])), material_item_ids: [...(phase.material_item_ids ?? [])], song_ids: [...(phase.song_ids ?? [])] }
}
        ...
        <div v-if="phases.length" class="list-group">
            ...
            <div v-for="(phase, index) in phases" :key="phase.id ?? phase.local_key" class="list-group-item">
                ...
                <div v-if="editing === (phase.id ?? phase.local_key)" class="border-top pt-3 mt-3">
                    ...
                    <PhaseResourcePicker :resources="resources" :resource-links="resourceLinks" :material-items="materialItems" :songs="songs" :selected-resource-ids="phaseForm.resource_ids" :selected-resource-link-ids="phaseForm.resource_link_ids" :resource-publication-statuses="phaseForm.resource_publication_statuses" :resource-link-publication-statuses="phaseForm.resource_link_publication_statuses" :selected-material-item-ids="phaseForm.material_item_ids" :selected-song-ids="phaseForm.song_ids" @update:resource-ids="phaseForm.resource_ids = $event" @update:resource-link-ids="phaseForm.resource_link_ids = $event" @update:resource-publication-statuses="phaseForm.resource_publication_statuses = $event" @update:resource-link-publication-statuses="phaseForm.resource_link_publication_statuses = $event" @update:material-item-ids="phaseForm.material_item_ids = $event" @update:song-ids="phaseForm.song_ids = $event" />
