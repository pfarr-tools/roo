...
const prompt = ref(''), model = ref('flux2-flex'), count = ref(1), removeWhite = ref(false), ratio = ref('1:1'), size = ref('1024'), credits = ref(null), results = ref([]), generating = ref(false), error = ref('')
...
async function choose(result) {
    ...
    try { const blob = await toPng(result.src, removeWhite.value); emit('selected', { blob, filename: 'flux-' + Date.now() + '.png', credits: modelLabel.value + ' / Black Forest Labs / ' + props.userName }); emit('close') } catch { error.value = 'Das FLUX-Bild konnte nicht verarbeitet werden.' }
}
