...
const hasItems = computed(() => props.resources.length || props.resourceLinks.length || props.materialItems.length || props.songs.length || materialTextItems.value.length)
...
function selectLibraryItem(item) {
    if ((item.kind === 'resource' || item.kind === 'material' || item.kind === 'song') && props.libraryAttachUrl && props.libraryTargetType && props.libraryTargetId) {
