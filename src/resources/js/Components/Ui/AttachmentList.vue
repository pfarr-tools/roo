...
const hasItems = computed(() => props.resources.length || props.resourceLinks.length || props.materialItems.length || props.songs.length || props.songbooks.length || materialTextItems.value.length)
...
function selectLibraryItem(item) {
    if ((item.kind === 'resource' || item.kind === 'material' || item.kind === 'song' || item.kind === 'songbook') && props.libraryAttachUrl && props.libraryTargetType && props.libraryTargetId) {
...
<template>
    ...
    <div v-if="hasItems" class="list-group">
        ...
        <article v-for="song in songs" :key="song.id || song.local_key" class="list-group-item d-flex align-items-start gap-2 py-2"><i class="bi bi-music-note-beamed mt-1" aria-hidden="true"></i><span class="flex-grow-1"><strong>{{ song.name || song.title }}</strong><span class="d-block small text-muted">Lied</span></span><button v-if="song.id" class="btn btn-sm btn-outline-danger" type="button" title="Löschen" @click="removeItem('song', song)"><i class="bi bi-trash"></i></button></article>
