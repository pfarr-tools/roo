...
const emit = defineEmits(['update', 'update:publication-status', 'delete', 'uploaded', 'select-resource', 'update:resource-links', 'update:material-items', 'update:songs', 'delete:resource-link', 'delete:material-item', 'error'])
...
const localResourceLinks = ref([...props.resourceLinks])
const localMaterialItems = ref([...props.materialItems])
const localAssessmentTasks = ref([...props.assessmentTasks])
const materialTextItems = computed(() => String(props.materialText ?? '').split('\n').map(item => item.trim()).filter(Boolean))
const hasItems = computed(() => localResources.value.length || localResourceLinks.value.length || localMaterialItems.value.length || props.songs.length || props.songbooks.length || localAssessmentTasks.value.length || materialTextItems.value.length)
...
watch(() => props.resources, resources => { localResources.value = [...resources] }, { deep: true })
watch(() => props.resourceLinks, resourceLinks => { localResourceLinks.value = [...resourceLinks] }, { deep: true })
watch(() => props.materialItems, materialItems => { localMaterialItems.value = [...materialItems] }, { deep: true })
watch(() => props.assessmentTasks, assessmentTasks => { localAssessmentTasks.value = [...assessmentTasks] }, { deep: true })
...
function uploadFile() {
    ...
    resourceUpload.submit('post', props.uploadUrl, { forceFormData: true, preserveScroll: true, onSuccess: page => { if (page?.props?.lesson?.resources) localResources.value = [...page.props.lesson.resources]; resourceUpload.reset('resource', 'description'); activeAdd.value = null; emit('uploaded', page) }, onError: errors => emit('error', Object.values(errors)[0] || de.uploadAttachmentError) })
}
...
function addMaterialItem() {
    ...
    if (props.libraryAttachUrl && props.libraryTargetType && props.libraryTargetId) {
        axios.post(`${props.libraryAttachUrl}/material/erstellen`, { ...materialItemForm.value, target_type: props.libraryTargetType, target_id: props.libraryTargetId }, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }).then(response => { localMaterialItems.value = [...localMaterialItems.value, response.data.item]; emit('update:material-items', [...localMaterialItems.value]); materialItemForm.value = { name: '', material_number: '', storage_location: '', description: '' }; activeAdd.value = null }).catch(error => emit('error', error.response?.data?.message || 'Das Material konnte nicht gespeichert werden.'))
        return
    }
...
function selectLibraryItem(item) {
    if ((item.kind === 'resource' || item.kind === 'material' || item.kind === 'song' || item.kind === 'songbook' || item.kind === 'assessment-task') && props.libraryAttachUrl && props.libraryTargetType && props.libraryTargetId) {
        axios.post(`${props.libraryAttachUrl}/${item.kind}/${item.id}/zuordnen`, { target_type: props.libraryTargetType, target_id: props.libraryTargetId }, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }).then(response => { if (item.kind === 'resource') { localResourceLinks.value = [...localResourceLinks.value, response.data.item || item]; emit('update:resource-links', [...localResourceLinks.value]) } if (item.kind === 'material') { localMaterialItems.value = [...localMaterialItems.value, response.data.item || item]; emit('update:material-items', [...localMaterialItems.value]) } if (item.kind === 'song') emit('update:songs', [...props.songs, response.data.item || item]); if (item.kind === 'assessment-task') { localAssessmentTasks.value = [...localAssessmentTasks.value, response.data.task || item]; emit('uploaded', response.data) } }).catch(error => emit('error', error.response?.data?.message || 'Die Zuordnung konnte nicht gespeichert werden.'))
        activeAdd.value = null
        return
    }
    if (item.kind === 'resource') { localResourceLinks.value = [...localResourceLinks.value, { ...item, local_key: `library-link-${item.id}` }]; emit('update:resource-links', [...localResourceLinks.value]) }
    if (item.kind === 'material') { localMaterialItems.value = [...localMaterialItems.value, { ...item, local_key: `library-material-${item.id}` }]; emit('update:material-items', [...localMaterialItems.value]) }
    ...
    if (item.kind === 'file') {
        ...
        axios.post(`${props.libraryAttachUrl}/${item.id}/zuordnen`, { target_type: props.libraryTargetType, target_id: props.libraryTargetId }, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }).then(response => {
            const resource = response.data.item || item
            if (!localResources.value.some(candidate => candidate.id === resource.id)) localResources.value = [...localResources.value, resource]
            emit('uploaded', response.data)
        }).catch(error => emit('error', error.response?.data?.message || 'Die Datei konnte nicht zugeordnet werden.'))
    }
    activeAdd.value = null
}
...
function saveEdit() {
    ...
    if (editingType.value === 'resource') { localResourceLinks.value = localResourceLinks.value.map(item => item === editingItem.value ? { ...item, title: editingForm.value.title, url: editingForm.value.url } : item); emit('update:resource-links', [...localResourceLinks.value]) }
    if (editingType.value === 'material') { localMaterialItems.value = localMaterialItems.value.map(item => item === editingItem.value ? { ...item, name: editingForm.value.name, material_number: editingForm.value.material_number, storage_location: editingForm.value.storage_location, description: editingForm.value.description } : item); emit('update:material-items', [...localMaterialItems.value]) }
    closeEdit()
}
function removeFromLocalList(type, item, page = null) {
    if (type === 'resource') { localResourceLinks.value = localResourceLinks.value.filter(candidate => candidate !== item); emit('update:resource-links', [...localResourceLinks.value]) }
    if (type === 'material') { localMaterialItems.value = localMaterialItems.value.filter(candidate => candidate !== item); emit('update:material-items', [...localMaterialItems.value]) }
    if (type === 'file') {
        localResources.value = localResources.value.filter(candidate => candidate.id !== item.id)
        emit('uploaded', { ...(page ?? {}), deletedResourceId: item.id })
    }
    if (type === 'assessment-task') {
        localAssessmentTasks.value = localAssessmentTasks.value.filter(candidate => candidate.id !== item.id)
        emit('uploaded', { ...(page ?? {}), deletedAssessmentTaskId: item.id })
    }
}
...
<template>
    ...
    <div v-if="hasItems" class="list-group">
        ...
        <article v-for="resource in localResources" :key="resource.id" class="list-group-item d-flex align-items-start gap-2 py-2"><i class="bi fs-5 text-primary mt-1" :class="iconFor(resource)" aria-hidden="true"></i><span class="flex-grow-1 min-w-0"><strong class="text-break">{{ resource.display_name || resource.original_name }}</strong><span class="d-block small text-muted">{{ fileTypeFor(resource) }} · {{ sizeFor(resource.size) }}<span v-if="isWscDoc(resource) && resource.page_count"> ({{ resource.page_count }} {{ pageLabel(resource) }})</span></span><span v-if="resource.description" class="d-block small text-muted text-pre-wrap mt-1">{{ resource.description }}</span><select v-if="manage && resource.lesson_id === null" class="form-select form-select-sm mt-2" :value="resource.publication_status || 'not_shared'" :aria-label="de.publicationStatus" @change="updatePublicationStatus(resource, $event.target.value)"><option value="not_shared">{{ de.publicationNotShared }}</option><option value="shared_immediately">{{ de.publicationImmediate }}</option></select></span><span class="d-flex gap-1"><button v-if="isPreviewable(resource)" class="btn btn-sm btn-outline-primary" type="button" title="Vorschau öffnen" @click="previewResource = resource"><i class="bi bi-eye" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-secondary" type="button" title="Bearbeiten" @click="openEdit('file', resource)"><i class="bi bi-pencil" aria-hidden="true"></i></button><a class="btn btn-sm btn-outline-secondary" :href="`${downloadBaseUrl}/${resource.id}/download`" title="Herunterladen"><i class="bi bi-download" aria-hidden="true"></i></a><button class="btn btn-sm btn-outline-danger" type="button" title="Löschen" @click="removeItem('file', resource)"><i class="bi bi-trash" aria-hidden="true"></i></button></span></article>
        <article v-for="link in localResourceLinks" :key="link.id || link.local_key" class="list-group-item d-flex align-items-start gap-2 py-2"><i class="bi bi-link-45deg mt-1" aria-hidden="true"></i><span class="flex-grow-1 min-w-0"><strong class="text-break">{{ link.title }}</strong><a class="d-block small text-break" :href="link.url" target="_blank" rel="noreferrer">{{ link.url }}</a></span><span class="d-flex gap-1"><button class="btn btn-sm btn-outline-secondary" type="button" title="Bearbeiten" @click="openEdit('resource', link)"><i class="bi bi-pencil" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-danger" type="button" title="Löschen" @click="removeItem('resource', link)"><i class="bi bi-trash" aria-hidden="true"></i></button></span></article>
        <article v-for="item in localMaterialItems" :key="item.id || item.local_key" class="list-group-item d-flex align-items-start gap-2 py-2"><i class="bi bi-box-seam mt-1" aria-hidden="true"></i><span class="flex-grow-1 min-w-0"><strong class="text-break">{{ item.name }}</strong><span v-if="item.material_number || item.storage_location" class="d-block small text-muted">{{ item.material_number ? `Materialnummer: ${item.material_number}` : '' }}{{ item.material_number && item.storage_location ? ' · ' : '' }}{{ item.storage_location ? `Lagerort: ${item.storage_location}` : '' }}</span><span v-if="item.description" class="d-block small text-muted text-pre-wrap">{{ item.description }}</span></span><span class="d-flex gap-1"><button class="btn btn-sm btn-outline-secondary" type="button" title="Bearbeiten" @click="openEdit('material', item)"><i class="bi bi-pencil" aria-hidden="true"></i></button><button class="btn btn-sm btn-outline-danger" type="button" title="Löschen" @click="removeItem('material', item)"><i class="bi bi-trash" aria-hidden="true"></i></button></span></article>
        ...
        <article v-for="task in localAssessmentTasks" :key="task.id" class="list-group-item d-flex align-items-start gap-2 py-2"><i class="bi bi-clipboard-check mt-1" aria-hidden="true"></i><span class="flex-grow-1"><strong>{{ task.title }}</strong><span class="d-block small text-muted">{{ assessmentTaskDescription(task) }}</span></span><button class="btn btn-sm btn-outline-danger" type="button" title="Löschen" @click="removeItem('assessment-task', task)"><i class="bi bi-trash"></i></button></article>
    </div>
