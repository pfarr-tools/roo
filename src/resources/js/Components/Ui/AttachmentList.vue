...
const libraryItems = computed(() => [
    ...props.libraryResources.map(item => ({ ...item, kind: 'file', label: item.original_name })),
    ...props.libraryResourceLinks.map(item => ({ ...item, kind: 'resource', label: item.title })),
    ...props.libraryMaterialItems.map(item => ({ ...item, kind: 'material', label: item.name })),
].filter(item => libraryType.value === 'all' || item.kind === libraryType.value).filter(item => String(item.label ?? '').toLowerCase().includes(librarySearch.value.toLowerCase())))
